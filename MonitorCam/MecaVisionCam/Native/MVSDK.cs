...
namespace MVSDK
{
    ...
    static public class MvApi
    {
        ...
        static MvApi()
        {
            ...
            // 加载失败时 LoadLibrary 不抛异常，静默返回 IntPtr.Zero，后续委托全为 null，
            // 首次调用会抛出难以定位的 NullReferenceException，此处显式给出可定位的报错
            if (hSdkDll == IntPtr.Zero)
                throw new DllNotFoundException(b64Bit
                    ? "无法加载麦格威 SDK 原生库 MVCAMSDK_X64.dll，请确认其已由 csproj CopyNative3rdDependToOutput 拷贝到输出目录"
                    : "无法加载麦格威 SDK 原生库 MVCAMSDK.dll，请确认其已部署到输出目录");

            CameraSdkInit = (pfnCameraSdkInit)GetFunctionAddress(hSdkDll, "CameraSdkInit", typeof(pfnCameraSdkInit));

            // 库已加载但导出缺失（典型：误部署例程 bin 的旧互操作组件），同样给出明确报错
            if (CameraSdkInit == null)
                throw new EntryPointNotFoundException("MVCAMSDK 库中未找到 CameraSdkInit 导出，请确认部署的是官方 SDK 原生库而非旧版互操作组件");
